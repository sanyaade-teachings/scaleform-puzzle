...
#include <Vision/Runtime/EnginePlugins/VisionEnginePlugin/Rendering/Postprocessing/ToneMapping.hpp>
  ...
  #include <Vision/Runtime/Base/System/IO/Stream/VFileServeStreamManager.hpp> //<IGNORE_PACKAGE_DEPENDENCY_WARNINGS
#endif
...
bool VisSampleApp::InitSample( const char *pszSampleDataDir, const char *pszSampleScene, uint64 iSampleFlags, int iX, int iY, int iEngineFlags, int iNumThreads)
{
  ...
  // Set the current directory to the EXE dir so the sample can find its data files
  ...
  {
    hkvLog::Warning("Both DRC demo and copy not possible at the same time, falling back to DRC demo!");
...
bool VisSampleApp::LoadScene(const char *pszSampleScene, int iAdditionalLoadingFlags, bool bAllowProfileFallback)
{
  ...
  {
    hkvLog::FatalError(m_sceneLoader.GetLastError());
    return false;
  }
...
bool VisSampleApp::AddSampleDataDir(const char *pszDataDir)
{
  if (pszDataDir && pszDataDir[0])
  {
    ...
    if (Vision::File.Exists(szTemp))
    {
      ...
      hkvLog::Info("Adding data directory \"%s\"", szTemp);
      return true;
    }
...
bool VisSampleApp::RemoveSampleDataDir(const char *pszDataDir)
{
  if (pszDataDir && pszDataDir[0])
  {
    ...
    if (VFileHelper::Exists(szTemp))
    {
      ...
      hkvLog::Info("Remove data directory \"%s\"", szTempFullPath);
      return true;
    }
...
void VisSampleApp::SetupBaseDataDir()
{
  ...
  {
    hkvLog::Info("Adding base data directory \"%s\"", VISION_BASE_DATA);
  }
  else
  {
    // No Base Data directory found (make this fatal?)
    hkvLog::Warning("Base Data directory (%s) not found", VISION_BASE_DATA);
  }
  ...
  #ifdef WIN32
  {
    ...
    if (VFileHelper::ExistsDir(szDir))
    {
      ...
      hkvLog::Info("Adding simulation data directory \"%s\"", szDir);
    }
  }
  #endif
}
...
bool VisSampleApp::SetupSampleData(const char *pszDataDir)
{
  ...
  {
    hkvLog::Info("Adding common data directory \"%s\"", VISION_COMMON_DATA);
  }
  else
  {
    hkvLog::Warning("Common Data directory (%s) not found", VISION_COMMON_DATA);
  }
...
void VisSampleApp::SetShadowsForLight(VisLightSource_cl *pLight, bool bStatus)
{
  ...
  {
    hkvLog::Warning("Renderer node 0 not set. VisSampleApp::SetShadowsForLight can not be used.");
    return;
  }
...
void VisSampleApp::OnLoadSceneStatus(int iStatus, float fPercentage, const char* pszStatus)
{
  ...
  case VIS_API_LOADSCENESTATUS_FINISHED:
    ...
    if (m_iInsideLoadWorldCounter==0) // this handles the loadworld inside the load scene
    {
      ...
      hkvLog::Info("Time for loading : %.2fs", fLoadingTime);
    }
    break;
...
#error "Platform not supported!"
  ...
  if(pszAlternativeCacheDir)
  {
    ...
    hkvLog::Info("Using alternative cache directory for vFileServe: %s", pszAlternativeCacheDir);
  }
  ...
  hkvLog::Info ("Searching for file-serve configuration in '%s'", sSettingsFilePath.AsChar ());
  ...
  if(pStream == NULL)
  {
    ...
    if(pStream == NULL)
    {
      hkvLog::Warning("Unable to find file serve configuration (please look up documentation for details)");
  ...
  if ( !bConnected )
  {
    hkvLog::Warning( "Unable to connect to file server!" );
  }
